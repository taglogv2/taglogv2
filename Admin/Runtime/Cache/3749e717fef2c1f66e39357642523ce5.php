...
<div class="container">
	<div class="row">
		<div class="span8">
			...
			<div class="hero-unit">
			<?php if(is_array($announcement_list)): $k = 0; $__LIST__ = $announcement_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><p><?php echo ($k); ?>、<a onclick="javascript:show(<?php echo ($vo['announcement_id']); ?>);" style="font-size: 14px;color:#<?php echo ($vo["color"]); ?>"><?php echo ($vo["title"]); ?></a> <?php if((time()-$vo['update_time']) < 86400*7): ?>&nbsp; <img src="./Public/img/new.gif"><?php endif; ?> &nbsp; (更新时间：<?php echo (date("Y-m-d H:i",$vo["update_time"])); ?>)
				</p>
		...
		<div class="span4">
		  <div style="border-left:1px solid #eee;"><form action="" method="post" class="form-signin">
			<fieldset>
			<legend><h3>管理员登录<small> - 用户宝</small></h3></legend>					
			<?php if(is_array($alert)): foreach($alert as $k=>$v): if(is_array($v)): foreach($v as $kk=>$vv): ?><div class="alert alert-<?php echo ($k); ?>">
			<button type="button" class="close" data-dismiss="alert">&times;</button>
			<?php echo ($vv); ?>
		</div><?php endforeach; endif; endforeach; endif; ?>
			用户名：<input type="text" name="name" class="text-input" placeholder="用户名"/><br/>
			密 &nbsp;&nbsp; 码：<input type="password" name="password" class="text-input" placeholder="密码"/>
			<br/>
			<label class="checkbox"><input type="checkbox" name="autologin"/> 三日内自动登陆</label>
			<input name="submit" class="btn btn-primary" type="submit" value="登录"/> &nbsp; <a href="<?php echo U('user/lostpw');?>">忘记密码</a>
			</fieldset>
